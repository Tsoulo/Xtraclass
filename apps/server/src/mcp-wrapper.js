...
- Consolidation and problem-solving
            ...
            try {
                switch (name) {
                    ...
                    case "generate_homework":
                        return await this.handleGenerateHomework(args);
    ...
    async handleGenerateExercise(args) {
        try {
            ...
            const exercise = await educationalAIService.generateExercise(context, numQuestions);
            return {
                content: [
                    {
                        type: "text",
    ...
    async handleGenerateFeedback(args) {
        try {
            ...
            const feedback = await educationalAIService.generateFeedback(exercise, studentAnswers, context);
            return {
                content: [
                    {
                        type: "text",
    ...
    async handleGenerateAdaptiveExercise(args) {
        try {
            ...
            const exercise = await educationalAIService.generateAdaptiveExercise(context, adaptiveFeedbackContext, numQuestions);
    ...
    async handleGenerateHomework(args) {
        try {
            const { context, requirements = {} } = args;
            if (!context) {
                throw new Error("Missing required parameter: context");
            }
            const homework = await educationalAIService.generateHomework(context, requirements);
            return {
                content: [
                    {
                        type: "text",
